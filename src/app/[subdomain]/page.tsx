          ...
          <div className="flex flex-col md:items-end justify-center gap-10">
            {config?.show_socials_footer && (
              <div className="flex gap-8">
                {config?.instagram && (
                  <a href={`https://instagram.com/${config.instagram}`} target="_blank" className="text-white hover:text-[#22c55e] transition-all"><Instagram size={28} /></a>
                )}
                {config?.whatsapp && (
                  <a href={`https://wa.me/${config.whatsapp}`} target="_blank" className="text-[#22c55e] hover:scale-110 transition-transform"><MessageCircle size={34} /></a>
                )}
              </div>
            )}
