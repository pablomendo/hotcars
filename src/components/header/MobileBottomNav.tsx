...
import { Home, Heart, Plus, Settings, Menu, X, User, LogOut } from 'lucide-react';
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[100] bg-[#12242e] border-t border-white/10 grid grid-cols-5 px-2 py-2" style={{ bottom: 0 }}>
            {isLoggedIn ? (
                ...
                <>
                    ...
                    <div className="flex items-center justify-center">
                        <button onClick={() => router.push('/login')} className="flex flex-col items-center justify-center w-14 h-14 rounded-full bg-[#134e4d] border-4 border-[#12242e] -mt-6 shadow-lg hover:opacity-90 active:scale-95 transition-all">
                            <Plus size={26} className="text-white" />
                        </button>
                    </div>
