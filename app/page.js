...
// Usamos la importación vía CDN para que el entorno de vista previa no falle al compilar
...
// --- BUSCADOR BLINDADO (SOLUCIÓN DEFINITIVA AL TECLADO) ---
// Definimos el buscador FUERA de la App principal. Esto evita que el input
// se destruya y se vuelva a crear cuando la lista de autos cambia.
const SearchBar = memo(({ onSearch }) => {
  ...
  return (
    <div className="relative max-w-xl mx-auto md:mx-0">
      ...
      <input 
        ref={inputRef}
        id="input-buscador-hotcars-definitivo"
        type="text" 
        inputMode="search"
        ...
        autoComplete="off"
        onFocus={(e) => {
           // Pequeño truco para asegurar que el cursor vaya al final en algunos navegadores móviles
...
export default function App() {
  ...
  const filteredCars = useMemo(() => {
    ...
    return INVENTORY_DATA.filter(car => 
      car.model.toLowerCase().includes(term) || car.brand.toLowerCase().includes(term)
    );
  }, [filter]);

  const handleWhatsApp = (car) => {
    const text = `¡Hola HotCars! Me interesa el ${car.brand} ${car.model}.`;
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans text-left selection:bg-orange-600 overflow-x-hidden">
      <nav className="h-20 bg-[#020617] border-b border-white/5 flex items-center justify-between px-6 md:px-12 sticky top-0 z-50">
        ...
        <button onClick={() => setShowAuthModal(true)} className="bg-orange-600 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-lg shadow-orange-600/20">
          Acceso Red
        </button>
      </nav>
      ...
      <header className="py-16 px-6 bg-gradient-to-b from-slate-900 to-[#020617]">
        <div className="max-w-6xl mx-auto">
          ...
          {/* El buscador se mantiene estable aquí */}
          <SearchBar onSearch={handleFilterUpdate} />
        </div>
      </header>
          ...
          {filteredCars.map((car) => (
            <div key={car.id} className="group bg-slate-900 rounded-[2.5rem] border border-white/5 overflow-hidden flex flex-col h-full shadow-lg hover:border-orange-600/30 transition-all duration-300">
              <div className="h-56 relative overflow-hidden cursor-pointer" onClick={() => setSelectedCar(car)}>
                <img src={car.image} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt={car.model} onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=800"; }} />
              ...
              <div className="p-7 flex flex-col flex-grow text-left">
                ...
                <div className="flex gap-3 mt-auto">
                  <button onClick={() => setSelectedCar(car)} className="flex-1 bg-slate-800 text-white py-4 rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 hover:bg-slate-700 transition-colors font-bold tracking-tighter"><Info size={14}/> Ficha</button>
                  <button onClick={() => handleWhatsApp(car)} className="flex-[2] bg-orange-600 text-white py-4 rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 active:bg-orange-700 transition-colors font-bold tracking-tighter"><MessageCircle size={14}/> Consultar</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
           <div className="bg-slate-900 border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden relative shadow-2xl animate-in fade-in zoom-in duration-200 text-left">
              ...
              <div className="p-10">
                ...
                <button onClick={() => handleWhatsApp(selectedCar)} className="w-full bg-orange-600 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest flex items-center justify-center gap-3"><MessageCircle size={20}/> Consultar via WhatsApp</button>
              </div>
           </div>
        </div>
      )}
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm text-left">
          <div className="bg-slate-900 border border-white/10 w-full max-w-md rounded-[2.5rem] p-10 relative shadow-2xl">
            <button onClick={() => setShowAuthModal(false)} className="absolute top-6 right-6 text-slate-500 hover:text-white"><X size={24} /></button>
            <div className="mb-8">
              ...
              <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest text-left">Acceso exclusivo para revendedores</p>
            </div>
