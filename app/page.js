...
// Importación vía CDN para asegurar que funcione tanto en Vercel como en la Vista Previa
...
// Definido fuera de la función Page para que React no lo destruya al actualizar estados.
...
const StatCard = ({ title, value, icon }) => (
  <div className="bg-slate-900/50 p-4 rounded-2xl border border-white/5 flex flex-col gap-2 hover:border-orange-600/20 transition-all text-left">
        ...
        <div className="flex items-center gap-4">
          {user ? (
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => supabase.auth.signOut()} className="text-slate-500 hover:text-orange-600 transition-all">
                <LogOut size={20}/>
              </button>
            </div>
          ) : (
                ...
                filteredCars.map(car => (
                  <div key={car.id} className="group bg-slate-900 rounded-[2.5rem] border border-white/5 overflow-hidden shadow-xl hover:border-orange-600/30 transition-all duration-500 text-left">
                    <div className="h-60 relative overflow-hidden cursor-pointer" onClick={() => setSelectedCar(car)}>
                      <img src={car.image || 'https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=800'} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt={car.model} />
                    ...
                    <div className="p-8 text-left">
                      <span className="text-orange-600 text-[10px] font-black uppercase tracking-widest font-bold">{car.brand}</span>
      ...
      {view === 'office' && user && (
        <main className="max-w-7xl mx-auto py-12 px-6">
          <div className="mb-12 border-b border-white/5 pb-12">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12 text-left">
              <div>
            ...
            {inventory.length > 0 ? (
              inventory.map(car => (
                <div key={car.id} className="bg-slate-900 border border-white/5 p-6 rounded-[2rem] text-left group hover:border-orange-600/20 transition-all">
                  ...
                  <div className="flex justify-between items-center mt-3 pt-3 border-t border-white/5 text-left">
              ...
              <div className="col-span-full py-24 text-center border-2 border-dashed border-white/5 rounded-[3rem]">
                ...
                <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] italic leading-loose">Tu oficina está vacía.<br/>Cargá tu primera unidad para publicarla.</p>
              </div>
            )}
          </div>
        </main>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-md">
          <div className="bg-slate-900 border border-white/10 w-full max-w-md rounded-[3rem] p-12 relative text-left shadow-2xl">
            ...
            <h2 className="text-3xl font-black italic uppercase text-white leading-none mb-10 text-left font-bold">ACCESO <br/><span className="text-orange-600">RED HOTCARS</span></h2>
           ...
           <div className="bg-slate-900 border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden relative shadow-2xl text-left animate-in fade-in zoom-in duration-200">
              ...
              <div className="p-10 text-left">
                ...
                <div className="grid grid-cols-2 gap-4 mb-10 text-left font-bold uppercase">
                  ...
                  <div className="bg-white/5 p-4 rounded-2xl text-left font-bold tracking-tighter uppercase font-bold"><p className="text-slate-500 text-[9px] font-black uppercase mb-1 tracking-widest font-bold">Kilometraje</p><p className="text-white font-bold text-sm uppercase">{selectedCar.km} KM</p></div>
                </div>
