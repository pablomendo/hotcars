...
import { 
  ...
  LogOut, Car, LayoutDashboard, Send
} from 'lucide-react';
...
// --- COMPONENTE BUSCADOR (FIX TECLADO DEFINITIVO) ---
// Definido FUERA de la función App para que React no lo destruya al actualizar la lista.
// Esto garantiza que el foco sea permanente y el teclado no se cierre nunca.
const GlobalSearch = memo(({ onSearch }) => {
  ...
  return (
    <div className="relative max-w-xl mx-auto md:mx-0">
      ...
      <input 
        id="buscador-hotcars-permanente"
        type="text" 
        inputMode="search"
...
export default function App() {
  ...
  const [inventory, setInventory] = useState([]); // Lista real de Supabase
  ...
  const fetchCars = async () => {
    ...
    try {
      ...
      // Si estamos en la oficina, mostramos solo los del usuario logueado
      if (view === 'office' && user) {
  ...
  // Recargar datos cuando cambia la vista o el usuario
  useEffect(() => {
    fetchCars();
  }, [view, user]);

  // Filtrado memoizado para no afectar el input
  ...
  const handleAuth = async (e) => {
    ...
    if (error) {
      ...
      else setStatusMsg({ text: 'Cuenta creada. Confirmá tu mail para entrar.', type: 'success' });
    } else {
        ...
        <>
          <header className="py-20 px-6 bg-gradient-to-b from-slate-900 to-[#020617] border-b border-white/5">
            <div className="max-w-6xl mx-auto text-left">
              <h1 className="text-5xl md:text-8xl font-black mb-10 leading-none tracking-tighter uppercase italic">
          ...
          <main className="max-w-7xl mx-auto py-12 px-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {loading ? (
                  ...
                  <div key={car.id} className="group bg-slate-900 rounded-[2.5rem] border border-white/5 overflow-hidden shadow-xl hover:border-orange-600/30 transition-all duration-500">
                    ...
                    <div className="p-8 text-left">
                      ...
                      <button 
                        ...
                        className="w-full bg-orange-600 text-white py-4 rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 active:bg-orange-700 transition-all shadow-lg shadow-orange-600/10"
                      >
                ...
                <div className="col-span-full text-center py-24 text-slate-700 font-black uppercase tracking-widest italic text-sm">
        ...
        <main className="max-w-7xl mx-auto py-12 px-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-8 border-b border-white/5 pb-12 text-left">
            ...
            <button 
              ...
              className="bg-white text-black px-10 py-5 rounded-[1.8rem] font-black uppercase text-[10px] tracking-widest flex items-center gap-3 hover:bg-orange-600 hover:text-white transition-all shadow-2xl active:scale-95"
            >
              <Plus size={20}/> Cargar Vehículo
            </button>
          </div>
            ...
            {inventory.length > 0 ? (
              ...
              <div className="col-span-full py-24 text-center border-2 border-dashed border-white/5 rounded-[3rem]">
                ...
                <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] italic">
                  Tu oficina está vacía.<br/>Cargá tu primera unidad para publicarla.
                </p>
              </div>
            )}
          </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-md">
          <div className="bg-slate-900 border border-white/10 w-full max-w-2xl rounded-[3rem] p-10 relative text-left shadow-2xl overflow-y-auto max-h-[90vh]">
            <button onClick={() => setShowAddModal(false)} className="absolute top-8 right-8 text-slate-500 hover:text-white"><X size={24} /></button>
            <h2 className="text-3xl font-black italic uppercase text-white mb-8">NUEVA <span className="text-orange-600">UNIDAD</span></h2>
            
            <form onSubmit={handleAddCar} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1 text-left">
                <label className="text-[9px] font-black uppercase text-slate-500 ml-2 tracking-widest font-bold">Marca</label>
              ...
              <div className="space-y-1 text-left">
                <label className="text-[9px] font-black uppercase text-slate-500 ml-2 tracking-widest font-bold">Modelo</label>
              ...
              <div className="space-y-1 text-left">
                <label className="text-[9px] font-black uppercase text-slate-500 ml-2 tracking-widest font-bold">Año</label>
              ...
              <div className="space-y-1 text-left">
                <label className="text-[9px] font-black uppercase text-slate-500 ml-2 tracking-widest font-bold">Precio (USD)</label>
                <input type="number" placeholder="55000" required className="w-full p-4 bg-slate-800 border-none rounded-2xl text-white font-bold outline-none focus:ring-2 focus:ring-orange-600" value={newCar.price} onChange={e => setNewCar({...newCar, price: e.target.value})} />
              </div>
              <div className="space-y-1 text-left">
                <label className="text-[9px] font-black uppercase text-slate-500 ml-2 tracking-widest font-bold">Kilómetros</label>
              ...
              <div className="space-y-1 text-left">
                <label className="text-[9px] font-black uppercase text-slate-500 ml-2 tracking-widest font-bold">URL de la Foto</label>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-md">
          <div className="bg-slate-900 border border-white/10 w-full max-w-md rounded-[3rem] p-12 relative text-left shadow-2xl">
            <button onClick={() => setShowAuthModal(false)} className="absolute top-8 right-8 text-slate-500 hover:text-white"><X size={24} /></button>
           ...
           <div className="bg-slate-900 border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden relative shadow-2xl text-left animate-in fade-in zoom-in duration-200">
              ...
              <div className="p-10 text-left">
                ...
                <div className="grid grid-cols-2 gap-4 mb-10 text-left">
                  <div className="bg-white/5 p-4 rounded-2xl text-left"><p className="text-slate-500 text-[9px] font-black uppercase mb-1 tracking-widest font-bold">Año</p><p className="text-white font-bold text-sm uppercase">{selectedCar.year}</p></div>
                  <div className="bg-white/5 p-4 rounded-2xl text-left"><p className="text-slate-500 text-[9px] font-black uppercase mb-1 tracking-widest font-bold">KMs</p><p className="text-white font-bold text-sm uppercase">{selectedCar.km}</p></div>
                </div>
